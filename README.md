...
`repo-ontology`는 프로젝트 루트에 기계 가독형 `.ontology/` 명세를 구축하고, `otlg lint`로 선언 필드·참조·코드 경로를 검사하며 `otlg trace`로 관련 요소를 찾습니다. 경로가 존재해도 문서의 동작 설명과 코드가 의미적으로 일치하는지는 별도 리뷰가 필요합니다.
